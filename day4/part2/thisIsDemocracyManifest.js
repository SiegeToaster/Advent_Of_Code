...
const raw = fs.readFileSync("day4/part1/input.txt", "utf-8");
...
for (let z = 0; z < drawNums.length; z++) {
	howtfAmIGoingToDoThis.forEach((board, key) => {
		...
		board.get('rows').forEach(value1 => {
			...
			checkWinner(board,'rows', arr, z, key);
		})
		board.get('columns').forEach(value2 => {
			...
			checkWinner(board, 'columns', arr, z, key);
		})
		// console.log('\n\nbreak\n\n')
	});
}
...
function checkWinner(board, type, arr, z, key) {
	if (!arr.includes(false) && winnerValues == null) {
		if (howtfAmIGoingToDoThis.size > 1) {
			return howtfAmIGoingToDoThis.delete(key)
		} else {
