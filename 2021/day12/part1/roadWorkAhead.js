...
function searchLevel(startCave, breadCrumbs) {
	...
	for (let i = 0; i < nextCaves.length; i++) {
		...
		if (
			nextCaves[i].toLowerCase() === nextCaves[i] // checks if cave is lowercase
		...
		if (nextCaves[i] === 'end') {
			...
			searchLevel(nextCaves[i], breadCrumbs);
		}
	}
}